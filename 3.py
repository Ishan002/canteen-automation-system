...
if task_id:
    ...
    if not task_data.empty:
        ...
        # Extract and print the start date, end date, assignedTo, and task name
                        ...
                        for user in matching_users:
                            if str(user['userId']) not in assigned_user_ids:
                                ...
                                if not user_info.empty:
                                    ...
                                    # Check user's other tasks and determine their earliest availability after the task's end date
                                    ...
                                    # Determine the latest end date of the user's current tasks
                                    latest_task_end_date = max((end_date for _, end_date in task_dates), default=None)

                                    if latest_task_end_date is not None:
                                        # Calculate time difference between user's latest task end date and current task's end date
                                        date_diff = latest_task_end_date - end_date
                                        if date_diff < nearest_date_diff:
...
#                                 if not user_info.empty:
#                                     print(f"User ID: {user['userId']}, Name: {user_info['firstName'].values[0]}, Specialization: {user['specilization']}")
#                                     available_users.append(user)  # Collect available users
...
#                                     if is_free:
#                                         # Find the closest date difference for the user's tasks
...
#                                 print(f"User ID: {user['userId']}, Specialization: {user['specilization']}")
                            
#                             # Prompt user to select a user ID
#                             selected_user_id = input("Please enter the User ID to whom you want to assign the task: ").strip()
#                             selected_user = next((user for user in available_users if str(user['userId']) == selected_user_id), None)
                            
#                             # Check if selected_user is a valid object and not empty
#                             if selected_user is not None:
#                                 user_info = user_table[user_table['id'] == selected_user['userId']]
#                                 if not user_info.empty:
#                                     print(f"\nTask {task_id} from {pd.Timestamp(start_date)} to {pd.Timestamp(end_date)} has been assigned to User ID: {selected_user['userId']}.")
#                                 else:
#                                     print("Invalid User ID. Task not assigned.")
#                             else:
#                                 print("Invalid User ID. Task not assigned.")
#                         else:
#                             print("\nNo available user found.")
