...
from pulp import *
from sqlalchemy import Column, String, create_engine, insert, DateTime
...
# =======Get data and convert to a DataFrame=======
    ...
    # holiday_data = pd.read_sql("SELECT id, userId,from_unixtime(floor(startDate/1000)) AS startDate, from_unixtime(floor(endDate/1000)) AS endDate FROM userVacation", engine)
    holiday_data = pd.read_sql("SELECT id, userId, from_unixtime(floor(startDate/1000)) AS startDate, from_unixtime(floor(endDate/1000)) AS endDate FROM userHoliday", engine)
    ...
    if schedule_by_time.empty:
       print("\nThere is no task or meeting scheduled for that day for that time period!")
        ...
        for i, row in schedule_by_time.iterrows():
            ...
            print(holiday_StartDate,holiday_EndDate)
